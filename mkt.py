...
def main():
    ...
    for tracker in args.trackers:
        ...
        subprocess.run([
            ...
            '--no-creation-date',
            '-a', tracker,
            '-o', d / f'{args.file.name}[{tracker_str}].torrent',
    ...
    p = subprocess.run([
        'mediainfo', file,
    ], cwd=args.file.parent, check=True, capture_output=True, encoding='utf-8')
    ...
    for i in range(args.snapshots):
        ...
        subprocess.run([
            ...
            '-v', 'error',
            '-stats',
            '-ss', str((60 if args.short else 300) * (i + 1)),
            '-i', files[i],
    ...
    for i in range(args.snapshots):
        ...
        with open(thumb, 'rb') as fd:
            ...
            res = r.json()
            img = snapshots[i]
            thumbnails += fr'[url={img}][img]https://ptpimg.me/{res[0]["code"]}.{res[0]["ext"]}[/img][/url]'
            if i % 2 == 0:
                thumbnails += ' '
            else:
                thumbnails += '\n'
